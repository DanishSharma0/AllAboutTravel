...
import {
  ...
  Info, Users, Languages, Award
} from 'lucide-react';

const SPECIALTIES = ['Local Culture', 'Trekking', 'History', 'Foodie', 'Adventure'];
const SORT_OPTIONS = [
...
const QUICK_FILTERS = [
  { label: 'All Guides', icon: Home, value: null },
  { label: 'Local Experts', icon: MapPin, value: { specialty: 'Local' } },
  { label: 'Trekking', icon: Award, value: { specialty: 'Trekking' } },
  { label: 'History', icon: Info, value: { specialty: 'History' } },
  { label: 'Top Rated', icon: Star, value: { sortBy: 'experience' } },
];
...
export default function TourGuides() {
  ...
  const fetchGuides = useCallback(async (activeFilters) => {
    setLoading(true);
    try {
      const params = {};
      if (activeFilters.city) params.city = activeFilters.city;
      if (activeFilters.minPrice && activeFilters.minPrice !== '0') params.minPrice = activeFilters.minPrice;
      if (activeFilters.maxPrice && activeFilters.maxPrice !== '8000') params.maxPrice = activeFilters.maxPrice;
      ...
      const response = await tourGuidesAPI.getAll(params);
  ...
  const handleApplyFilters = () => fetchGuides(filters);
        ...
        <div className="max-w-[1600px] mx-auto px-6 flex items-center justify-between gap-10">
          <div className="flex items-center gap-6 shrink-0">
            <h1 className="text-xl font-black text-slate-900 tracking-tight leading-none">
              Expert <span className="text-accent-500">Guides</span>
            </h1>
          </div>
          <div className="flex-1 max-w-2xl">
            <form onSubmit={handleSearchSubmit} className="relative group">
              ...
              <input
                ...
                placeholder="Find a guide in your city..."
        ...
        <aside className="hidden lg:block shrink-0 w-80">
          <div className="sticky top-32 flex flex-col gap-5">
            ...
            <div className="hostel-sidebar-scroll overflow-y-auto max-h-[calc(100vh-14rem)] flex flex-col gap-4 pr-2">
                    ...
                    return (
                      <button
                        ...
                        className={`py-3 rounded-2xl border text-[10px] font-black transition-all ${
                          isActive ? 'bg-slate-900 border-slate-900 text-white shadow-lg' : 'bg-sand-50/50 border-sand-100 text-slate-500 hover:border-accent-200'
                        }`}
                      >
                        {exp.label}
                      </button>
              ...
              <div className="bg-white rounded-[2rem] p-6 border border-sand-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.02)] space-y-5">
                <div className="flex items-center gap-2 text-[10px] font-black text-slate-900 uppercase tracking-[0.2em]">
                  <Languages size={12} className="text-accent-500" />
                  Guide Specialty
                </div>
                <div className="space-y-2">
                  {SPECIALTIES.map((spec) => {
                    ...
                    return (
                      <button
                        key={spec}
                        onClick={() => handleFilterChange('specialty', isActive ? '' : spec)}
        ...
        <main className="flex-1">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
              ...
              <h2 className="text-xs font-black text-slate-900 uppercase tracking-[0.2em]">
                {filters.city ? `Experts in ${filters.city}` : "Mountain Pathfinders"}
              </h2>
            </div>
          </div>
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-48 gap-6">
              ...
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.5em] animate-pulse">Consulting Experts...</p>
            </div>
          ) : guides.length === 0 ? (
            <div className="bg-white rounded-[3rem] border border-sand-200 p-24 text-center shadow-sm">
              ...
              <h3 className="text-lg font-black text-slate-900 mb-2 uppercase tracking-widest">No guides found</h3>
